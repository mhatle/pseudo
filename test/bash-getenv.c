 ...
 *
 * This file is a verbatim copy of bash-5.1's lib/sh/getenv.c, taken from
 * the Rocky Linux 9.3 bash sources, with only two mechanical changes:
 *
 *   1. The bash-internal include block (config.h, bashansi.h, shell.h)
 *      is replaced by "bash-getenv-shim.h", which supplies the small
 *      subset of the bash variable-subsystem interface this file needs.
 *
 *   2. When compiled, we add -Wno-nonnull-compare, to silence warnings
 *      about defensive "name == 0" guards below because glibc declares
 *      getenv/setenv/unsetenv with the 'nonnull' attribute.
 *      The guards are bash's own and are kept intentionally.
 *
 * The bodies of getenv(), _getenv(), putenv(), setenv() and unsetenv()
 * are unchanged from upstream bash.
 */
